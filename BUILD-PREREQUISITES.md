...
apt-get update
apt-get -y install docker-ce openjdk-8-jdk git
...
wget --no-check-certificate https://nodejs.org/dist/v8.11.1/node-v8.11.1-linux-x64.tar.xz && tar xf node-v8.11.1-linux-x64.tar.xz && rm node-v8.11.1-linux-x64.tar.xz

export PATH=$HOME/xqa/bin/apache-maven-3.5.3/bin:$HOME/xqa/bin/node-v8.11.1-linux-x64/bin:$PATH

echo "" >> $HOME/.bashrc
echo "export PATH=$HOME/xqa/bin/apache-maven-3.5.3/bin:$HOME/xqa/bin/node-v8.11.1-linux-x64/bin:$PATH" >> $HOME/.bashrc
```
...
# wait ~120 seconds / docker logs <dev_xqa-ingest_1 | dev_xqa-shard_1>
